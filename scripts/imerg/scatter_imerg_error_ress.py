...
if __name__ == '__main__':
    ...
    error_path = '/data/thesis/data_analysis/opera/3tiff_q_d/2015'
    residual_path = '/data/thesis/data_analysis/residuals/2res_OPERA_masked/2015'
    imerg_path = '/data/thesis/data_analysis/opera/3tiff_p_d/2015'
    
    error_p_fns = find_matching_days(error_path, imerg_path, residual_path)

    errors = []
    imerg_list = []
    ress = []

    for date, fns in error_p_fns.items():
        
        error_fn, imerg_fn, residual_fn = fns
        
        imerg_array = getArray(imerg_fn)
        error_array = getArray(error_fn)
        residual_array = getArray(residual_fn)
        
        for i in range(len(error_array)):
            for j in range(len(error_array[i])):
                
                imerg = imerg_array[i][j]
                error = error_array[i][j]
                residual = residual_array[i][j]

                if error != -9999000 and imerg > 5 and residual != -9999000:
                    
                    imerg_list.append(imerg)
                    errors.append(error)
                    ress.append(residual)

    # Create plot
    fig = plt.figure()
    ax = fig.add_subplot(111)
    
    data = ax.hist2d(errors, ress, norm=LogNorm(), cmin=1,  cmap = 'viridis',  bins = 75)
    
    #ax.scatter(randomError, residuals, alpha=0.8, c=z, edgecolors='none', s=3)
    #ax.plot(randomError, y_pred)
    
    plt.xlabel('OPERA Qind [-]')
