...
def calculate_average(imerg_path, opera_path, output_path, weigth=0.5):
    ...
    for date, (filename_imerg, filename_opera) in matching_pairs.items():
        ...
        for i in range(len(imerg)):
            for j in range(len(imerg[i])):
                ...
                if opera_value > 1 and imerg_value != -9999000:
                    average[i][j] = (imerg_value * weigth) + (opera_value * (1 - weigth))
                elif opera_value >= 0 and imerg_value != -9999000:
...
YEAR = '2017'
WEIGTH = [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4,0.45,0.5,0.55,0.6]
...
for weigth in WEIGTH:
    ...
    calculate_average(IMERG_PATH, OPERA_PATH, OUTPUT_PATH, weigth)
    ...
    results[weigth] = [far, pod, rmse, rel_bias, cor]
    
    print('\nSummary for weigth, '+str(weigth)+':')
