...
if __name__ == '__main__':
    ...
    cth_path = '/home/bram/Data/thesis/data_analysis/MSG/4OPERA_tov_mv/2015'
    #cth_path ='/home/bram/Data/thesis/data_analysis/MSG/3align_opera/2015/cth'

    qind_res_cth_fns = find_matching_days(qind_path, residual_path, cth_path)

    qinds = []
    cths = []
    ress = []

    for date, fns in qind_res_cth_fns.items():
        
        qind_fn, residual_fn, cth_path = fns
        ...
        for i in range(len(qind_array)):
            for j in range(len(qind_array[i])):
                ...
                if qind != -9999000 and res != -9999000 and cth != -9999000:
    ...
    data = ax.hist2d(qinds, cths,  norm=LogNorm(), cmap = 'viridis',  bins = 75)
        ...
        for i in range(len(qind_array)):
            for j in range(len(qind_array[i])):
                ...
                if qind != -9999000 and res != -9999000 and cth != -9999000:
                    ...
                    if qind <= 0.2:
                        qinds[0].append((res, cth))
                    
                    elif qind <= 0.4:
                        qinds[1].append((res, cth))
                    
                    elif qind <= 0.6:
                        qinds[2].append((res, cth))
        
                    elif qind <= 0.8:
                        qinds[3].append((res, cth))
                        
                    elif qind <= 1:
                        qinds[4].append((res, cth))                        
   ...
   # plt.scatter(x1, y1, alpha=0.5)
    ...
    for i in range(2):
        for j in range(3):
            data = ax[i, j].hist2d(x[count], y[count], cmap = 'viridis', norm=LogNorm(), bins = 75,range=([0, 14000], [-100, 100]))
            count += 1
            if count == 5:
                break
